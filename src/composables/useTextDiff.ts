...
// segment-level diff grouped into aligned change rows (for side-by-side rendering)
...
// tiny composable wrapper so components can `const { diffWords } = useTextDiff()`
export function useTextDiff() {
	return { diffWords, diffRows, splitSegments };
}
